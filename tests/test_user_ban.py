...
def test_banned_phone_is_detected(monkeypatch):
    monkeypatch.setenv("CONJECTA_BANNED_PHONES", "19074643721")
    user_ban.clear_ban_cache()

    assert user_ban.is_phone_banned("19074643721") is True
    assert user_ban.is_user_banned(user_id=user_id_for_phone("19074643721")) is True
